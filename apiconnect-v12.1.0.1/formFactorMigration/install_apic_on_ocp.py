...
#CD RELEASE
CD_CP4I_VERSION = "16.1.2" # check for correct values before GA
CD_PN_CHANNEL = "v8.1" # check for correct values before GA
CD_APIC_CHANNEL = "v7.0" # check for correct values before GA
CD_DP_CHANNEL = "v1.15"  # check for correct values before GA
CD_APIC_VERSION = "12.1.0.1" # check for correct values before GA
...
def install_catalog_sources_and_subscribe():
    ...
    for eachSubscription in subscriptions:
        if "datapower" in eachSubscription:
            ...
            if channel >= dpChannel:
                ...
                print("ACTION OUTPUT : Datapower operator already subscribed for channel number, " + channel + " Not installing again. Proceeding further.")
                print()
            else:
                print (ERRORED_TIME_MESSAGE, time.asctime())
                sys.exit("Datapower operator already subscribed for invalid channel number, " + channel + ". Uninstall it and try again. Supported channel versions are >=" + dpChannel)
    ...
    if dpSubscriptionExists == False:
        ...
        # use the args.dp_operator_channel if provided
        if args.dp_operator_channel != None:
            if (args.dp_operator_channel < dpChannel):
                print (ERRORED_TIME_MESSAGE, time.asctime())
                sys.exit("ERROR :Invalid channel version provided to install Datapower operator. Please provide the supported channel version in the v1.xx format. Given value is {}. Exiting...".format(args.dp_operator_channel))
            else:
                datapower_subscribe["spec"]["channel"] = args.dp_operator_channel
...
parser.add_argument("-dp_operator_channel", "--dp_operator_channel", dest = "dp_operator_channel", help="Channel version of Datapower Operator. If not provided, will default to value of the variable, CD_DP_CHANNEL")
...
print ("Datapower Operator Channel (-dp_operator_channel or --dp_operator_channel): ", args.dp_operator_channel)
