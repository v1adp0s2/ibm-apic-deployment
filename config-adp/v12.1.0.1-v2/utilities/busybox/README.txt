...
kubectl delete cluster -n apic management-<CLUSTER-ID>-db

# Step 4: Edit clear-pvc-pod.yaml — set the PVC names:
#   claimName: management-<CLUSTER-ID>-db-1
#   claimName: management-<CLUSTER-ID>-db-1-wal

# Step 5: Apply busybox pod
kubectl apply -f clear-pvc-pod.yaml

# Step 6: Watch until pod is Completed
kubectl get pod busybox-clear-pvc -n apic -w

# Step 7: Check logs to confirm cleared
kubectl logs -n apic busybox-clear-pvc

# Step 8: Delete busybox pod
kubectl delete pod busybox-clear-pvc -n apic

# Step 9: Delete the PVCs
kubectl delete pvc -n apic management-<CLUSTER-ID>-db-1
kubectl delete pvc -n apic management-<CLUSTER-ID>-db-1-wal

# Step 10: Re-apply Management CR (will recreate DB from scratch)
kubectl apply -f ../../core/03-management/06-management-cr.yaml -n apic
...
The busybox image must be available in your registry:
  harbor.adp.example.com/apic/busybox:1.37
